...
//	initCardForms();
//	initAreaInfoForms();
//	initContactInfoForms();
//	initAccountsManagerInfoForms();
//	initotherInfoForms();
//	initauditorForms();
...
//	  var customerId = decodeURI(loc.substr(n2+1, n1-n2));//从=号后面的内容
	...
	function initTest(){
		var Range = 9999999 - 1000000;   
		var Rand = Math.random();   
		var num =  1000000 + Math.round(Rand * Range)
				...
				[{
					...
					"data": [{
						...
						"callbackClick": function (self, _this) {
							var formData = self.jsonData();
							var validate = self.validate();
												
								var resultJsonObject={};
								for(var attr in formData.l1){
								resultJsonObject[attr]=formData.l1[attr];
								}
								for(var attr in formData.l2){
								resultJsonObject[attr]=formData.l2[attr];
								}
								for(var attr in formData.l3){
									resultJsonObject[attr]=formData.l3[attr];
									}
								for(var attr in formData.l4){
									resultJsonObject[attr]=formData.l4[attr];
									}
								for(var attr in formData.l5){
									resultJsonObject[attr]=formData.l5[attr];
									}
...
//							/saveCmCustomerInfo
							commonData.postData({
								serviceUrl: "/data/saveCmCustomerInfo",
								...
								callback: function (result) {
									if(result.customerId != null){
										alert("添加成功");	
										window.location.href='GroupMemberInfo.html?customerId='+ result.customerId;
									}
								}
							});
